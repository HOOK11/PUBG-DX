...
// protection
VMP_IMPORT void VMP_API VMProtectBegin(const char *);// 开始常规保护
VMP_IMPORT void VMP_API VMProtectBeginVirtualization(const char *);// 开始虚拟保护
VMP_IMPORT void VMP_API VMProtectBeginMutation(const char *);// 开始变异保护
VMP_IMPORT void VMP_API VMProtectBeginUltra(const char *);// 开始超级保护(虚拟保护 + 变异保护)
VMP_IMPORT void VMP_API VMProtectBeginVirtualizationLockByKey(const char *);// 开始虚拟按键保护
VMP_IMPORT void VMP_API VMProtectBeginUltraLockByKey(const char *);// 开始超级按键保护
VMP_IMPORT void VMP_API VMProtectEnd(void);// 结束保护(上面的每一个函数都要对应一个结束保护)

// utils
VMP_IMPORT BOOLEAN VMP_API VMProtectIsProtected();// 判断是否处于保护代码中
VMP_IMPORT BOOLEAN VMP_API VMProtectIsDebuggerPresent(BOOLEAN); // IRQL = PASSIVE_LEVEL// 判断是否处于调试器中
VMP_IMPORT BOOLEAN VMP_API VMProtectIsVirtualMachinePresent(void); // IRQL = PASSIVE_LEVEL// 判断是否处于虚拟机中
VMP_IMPORT BOOLEAN VMP_API VMProtectIsValidImageCRC(void);// 判断程序CRC是否正确
